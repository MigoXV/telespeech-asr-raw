...
from .audio_pretraining import SpecPretrainingTask, SpecPretrainingConfig
...
@dataclass
class SpecFinetuningConfig(SpecPretrainingConfig):
...
@register_task("spec_finetuning", dataclass=SpecFinetuningConfig)
class SpecFinetuningTask(SpecPretrainingTask):
    """ """

    cfg: SpecFinetuningConfig
    ...
    def load_dataset(
        self, split: str, task_cfg: SpecFinetuningConfig = None, **kwargs
    ):
        ...
        REGEX_TOKEN = re.compile(r"token:")
        REGEX_SPLIT = re.compile(r"\t")
        if task_cfg.multi_corpus_keys is None:
            label_path = os.path.join(data_path, f"{split}.tsv")
            ...
            with open(label_path, "r") as f:
                for i, line in enumerate(f):
                    ...
                    tokens = REGEX_TOKEN.sub("", items[4])
                    labels.append(text_compressor.compress(tokens))
            ...
            for key, file_name in data_keys:
                k = key.strip()
                label_path = os.path.join(data_path, f"{file_name.strip()}.tsv")
